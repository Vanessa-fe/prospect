...
export function InteractionForm({ contactId }: InteractionFormProps) {
  ...
  // Déterminer si on a besoin de certains champs selon le type
  ...
  const onSubmit = async (data: CreateInteractionInput) => {
    ...
    if (!result.success) {
      ...
      reset({ contactId, occurredAt: new Date() })
      router.refresh()
    }
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Enregistrez un appel, un message ou une note concernant ce contact
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="space-y-4 py-4">
            ...
            {/* Canal (optionnel sauf pour certains types) */}
            <div className="space-y-2">
